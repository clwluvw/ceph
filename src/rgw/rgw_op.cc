...
int RGWGetObj::verify_permission(optional_yield y)
{
  ...
  if (int ret = verify_object_permission(this, s, action); ret < 0) {
    return ret;
  }
                                       ...
                                       bool swift_slo)
{
  ...
  if (s->system_request) {
    ...
    ldpp_dout(this, 2) << "overriding permissions due to admin operation" << dendl;
				       ...
				       s->iam_user_policies, s->session_policies, action); ret < 0) {
    if (ret == -EACCES)
      return -EPERM;
...
int RGWCreateBucket::verify_permission(optional_yield y)
{
  ...
  ARN arn = ARN(bucket);
  if (int ret = verify_user_permission(this, s, arn, rgw::IAM::s3CreateBucket, false); ret < 0) {
    return ret;
  }
