...
void usage()
{
  ...
  cout << "   --log-zone-id=<id>                target zone id for per-zone datalog operations\n";
...
int main(int argc, const char **argv)
{
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
    ...
    } else if (ceph_argparse_witharg(args, i, &val, "--log-zone-id", (char*)NULL)) {
      opt_log_zone_id = rgw_zone_id(val);
