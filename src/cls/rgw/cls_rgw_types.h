...
struct rgw_usage_info {
  ...
  rgw_usage_info() {}
  rgw_usage_info(std::string category, std::string ip_tag)
    : category(category), ip_tag(ip_tag) {}
  rgw_usage_info(std::string category)
    : category(category) {}
  ...
  bool operator<(const rgw_usage_info& ui2) const {
    int comp = category.compare(ui2.category);
    if (comp < 0)
      return true;
    else if (!comp)
      return ip_tag.compare(ui2.ip_tag) < 0;
